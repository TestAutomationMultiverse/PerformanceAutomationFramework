...
/**
 * A result container within the ECS architecture that can represent either a successful result or a failure.
 * This follows the functional programming pattern for error handling.
 * 
...
public class Result<T> implements Serializable {
    ...
    /**
     * Create a failure result from an EcsException
     * @param exception The exception that caused the failure
     * @param <T> Type of the successful result
     * @return A failure result
     */
    public static <T> Result<T> failure(EcsException exception) {
     ...
     * @throws EcsException If the result is a failure
     */
    public T getValue() {
        if (isFailure()) {
            throw new EcsException(errorCode, errorMessage, exception);
        }
        return value;
    }
