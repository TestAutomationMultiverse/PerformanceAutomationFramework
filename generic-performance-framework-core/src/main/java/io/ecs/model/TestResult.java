...
public class TestResult {
    ...
    /**
     * Set error message (alias for setError for compatibility)
     *
    ...
    /**
     * Store additional response data as a map
     *
     * @param responseData Map of response data attributes
     */
    public void setResponseData(Map<String, Object> responseData) {
        // Since we don't have a dedicated field for generic response data,
        // we'll convert and store relevant information in existing fields
        if (responseData != null) {
            if (responseData.containsKey("contentLength") && responseData.get("contentLength") instanceof Number) {
                this.receivedBytes = ((Number) responseData.get("contentLength")).longValue();
            }
            
            if (responseData.containsKey("contentType") && responseData.get("contentType") instanceof String) {
                // Store content type in headers
                if (this.headers == null) {
                    this.headers = new HashMap<>();
                }
                this.headers.put("Content-Type", (String) responseData.get("contentType"));
            }
        }
    }
